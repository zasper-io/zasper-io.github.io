...
Zasper replaces JupyterLab by reimplementing most of the implementations. 

JupyterLab's wire protocol is built using Tornado server and AsyncIO. Python's AsyncIO runs on a single core and gets problematic when scaling.
...
Even if you read the code, Zasper's implementation is much easier to understand than JupyterLab. You wouldn't get lost in Future and callbacks.
